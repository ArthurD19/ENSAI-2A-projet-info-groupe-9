...
def test_ajouter_pot(comptage, table_exemple):
    """Ajoute le pot total de la table au comptage."""
    table_exemple.pot = 200
    comptage.ajouter_pot(table_exemple)
    assert comptage.pot == 200


def test_ajouter_pot_perso(comptage, table_exemple):
    """Ajoute une mise spécifique pour un joueur."""
...
def test_distrib_pots(comptage, table_exemple):
    """Vérifie la distribution correcte des pots entre gagnants."""
    alice, bob = table_exemple.joueurs
    comptage.ajouter_pot_perso(alice, 100)
    comptage.ajouter_pot_perso(bob, 200)

    gains = comptage.distrib_pots([alice, bob])
    assert alice in gains
    assert bob in gains
    assert sum(gains.values()) == comptage.pot or sum(gains.values()) > 0
